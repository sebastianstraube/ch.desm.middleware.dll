package ch.desm.middleware.modules.component.interlocking.obermattlangnau.maps;

import java.util.Map;

import ch.desm.middleware.modules.component.ComponentMapBase;

public class OMLMapUbw32Analog extends ComponentMapBase {

	//B0 = AN0, B1 = AN1
	//11bin = 3dez 
	public static final String PINBITMASK_INPUT_ANALOG = "3";
	
	public OMLMapUbw32Analog(){
		super();
	}
		
	public boolean isKeyAvailable(String id){
		return map.containsKey(id);
	}
	
	public boolean isValueAvailable(String id){
		return map.containsValue(id);
	}
	
	@Override
	public Map<String, String> getMap(){
		return this.map;
	}
	
	public void initialize(){
//		map.put("8.91.01", "B0#463#472"); //FSS Grundstellung
//		map.put("8.91.03", "B0#419#427"); //FSS 10° F 
//		map.put("8.91.04", "B0#392#400"); //FSS 30° F 
//		map.put("8.91.05", "B0#351#360"); //FSS 45° F 
//		map.put("8.91.06", "B0#298#306"); //FSS 80° F 
//		map.put("8.91.07", "B0#239#242"); //FSS 90° F 
//		map.put("8.91.19", "B0#498#511"); //FSS 10° EG 
//		map.put("8.91.20", "B0#524#532"); //FSS 30° EG 
//		map.put("8.91.21", "B0#572#580"); //FSS 45° EG 
//		map.put("8.91.22", "B0#639#652"); //FSS 80° EG 
//		map.put("8.91.23", "B0#669#680"); //FSS 90° EG 		
		
		map.put("8.91.01", "B0"); //FSS Grundstellung
		map.put("8.91.03", "B0"); //FSS 10° F 
		map.put("8.91.04", "B0"); //FSS 30° F 
		map.put("8.91.05", "B0"); //FSS 45° F 
		map.put("8.91.06", "B0"); //FSS 80° F 
		map.put("8.91.07", "B0"); //FSS 90° F 
		map.put("8.91.19", "B0"); //FSS 10° EG 
		map.put("8.91.20", "B0"); //FSS 30° EG 
		map.put("8.91.21", "B0"); //FSS 45° EG 
		map.put("8.91.22", "B0"); //FSS 80° EG 
		map.put("8.91.23", "B0"); //FSS 90° EG 		
	}
}
