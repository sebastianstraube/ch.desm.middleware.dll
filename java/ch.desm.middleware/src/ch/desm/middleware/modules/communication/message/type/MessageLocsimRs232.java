 ...
 * & vbCrLf) End If Case "09" ' Halte relais M-Taste ' Sollte über 40 km/h
 * auslösen, bekommen aber nichts vom LOCSIM, habe darum in der Geschwidnigkeit
 ...
 * 
 * ' M-Taste Nicht Gedrückt ... If CheckBox3.Checked Then
 * MyFile.Write("Got Quittung Gelb from Loksim and M-Taste is NOT gedrückt" &
 * Daten & vbCrLf) End If ' Make sure we ignore further "quit gelb" for 8
 * seconds IgnoreZusiForNow = True IgnoreZusi.Enabled = True
 * 
 * ' check if Vorgesorgt:
 * 
 * If Form2.Button25.Tag = 2 Then ' Rückstelltaste bereits gedrückt
 * SendToCabine(&H5D, &HC0, 5, 0, 1) If CheckBox3.Checked Then
 * MyFile.Write("Rückstelltaste bereits gedrückt" & Daten & vbCrLf) End If Else
 * ' Rückstelltaste nicht gedrückt SendToCabine(&H5D, &HC0, 6, 0, 1)
 * ZusiWarteAufQuittung = True If CheckBox3.Checked Then
 * MyFile.Write("Rückstelltaste nicht gedrückt" & Daten & vbCrLf) End If End If
 * Else ' M-Taste ist gedrückt ... SendToCabine(&H5D, &HC0, 3, 0, 1) If
 * CheckBox3.Checked Then
 * MyFile.Write("Got Quittung Gelb from Loksim and have M-Taste gedrückt" &
 * Daten & vbCrLf) End If ' Make sure we ignore further "quit gelb" for 8
 ...
 * End If ' Quitung Rot ' Hauptsignal überfahren If Daten <> 0 Then If
 * Form2.Button18.Tag = 1 Then SendToCabine(&H5D, &HC0, 1, 0, 1) If
 * CheckBox3.Checked Then
 * MyFile.Write("Got Quittung Rot from Loksim and have M-Taste NOT gedrückt" &
 * Daten & vbCrLf) End If Else ' M-Taste ist gedrückt ... SendToCabine(&H5D,
 * &HC0, 3, 0, 1) If CheckBox3.Checked Then
 * MyFile.Write("Got Quittung Rot from Loksim and have M-Taste gedrückt" & Daten
 * & vbCrLf) End If End If
 * 
 * Else ' Signale Reset!! Zusi zuürcksetzen SendToCabine(&H5D, &HC0, 0, 0, 1)
 * End If
 * 
 * 
 ...
 * Case "D" ' Diskretisierungs Wert... (Hysterese für Analogkanäle) Case Else
...
public class MessageLocsimRs232 extends MessageBase {
	 ...
	 */
	public MessageLocsimRs232(String payload, String topic) {
		super(payload, topic);
