...
extern "C" {
	...
	JNIEXPORT void JNICALL Java_ch_desm_Dll_getBalise0(JNIEnv* env, jobject obj, jint baliseId, jobject al)
	{
		int stellung;
		char protokoll[1024];
		int protokollLen;
		int rc = stw_getBalise(baliseId, &stellung, &protokoll[0], 1024, &protokollLen);
